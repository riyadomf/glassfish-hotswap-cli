...
*Building a command-line hot-swap workflow for GlassFish using JVM debugging internals, without IntelliJ Ultimate, JRebel, or a custom JVM.*
...
2. Save.
3. Rebuild the WAR.
4. Wait for GlassFish to undeploy the old version.
5. Wait for it to deploy the new one.
6. Wait for CDI, EJBs, JPA, JSF, and everything else to wake back up.
